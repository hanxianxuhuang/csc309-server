...
app.use((err, req, res, next) => {
    if (err instanceof multer.MulterError) {
        if (err.code === "LIMIT_FILE_SIZE") {
            ...
            res.status(400).json({ message: "Unexpected file is uploaded. Please check if your URL and request method are correct." });
        } else {
            console.error(err);
            console.log(req);
            res.status(501).json({ message: "Upload fails. Please contact system administrator." });
        }
    } else if (err instanceof SyntaxError) {
        return res.status(400).send({ message: "Invalid data is received." });
    } else {
        console.error(err);
        res.status(500).json({ message: "Unknown error. Please contact system administrator." });
    }
})
